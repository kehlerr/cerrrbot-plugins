...
class ReminderParamsExtracted(BaseModel):
    ...
    is_next_week: bool | None = Field(
        default=False, description="Set to true ONLY if the user explicitly says 'next' or 'next week'."
    )

    start_in_unit: TimeUnit | None = Field(
        default=None, description="Unit of time for relative start intervals. Use this for 'in X' (через X)."
    )

    start_in_value: int | None = Field(
        default=None, description="Numeric value for the relative start interval (e.g., 10 for 'in 10 days')."
    )
    ...
    repeat_value: int | None = Field(default=None, description="Interval value for repeating tasks (e.g., 1 for 'every day').")
